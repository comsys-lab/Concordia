...
class double_buffered_scratchpad:
    def __init__(self):
        self.ifmap_PartofVMEM_buf = rdbuf()
        self.filter_PartofVMEM_buf = rdbuf()
        self.ifmap_PartofCMEM_buf = rdbuf()
        self.filter_PartofCMEM_buf = rdbuf()
    ...
    def set_params(self,
        ...
        if self.estimate_bandwidth_mode:
            self.ifmap_PartofVMEM_buf = rdbuf_est()
            self.ifmap_PartofCMEM_buf = rdbuf_est()
            self.filter_PartofVMEM_buf = rdbuf_est()
            self.filter_PartofCMEM_buf = rdbuf_est()

            self.ifmap_PartofVMEM_buf.set_params(backing_buf_obj=self.ifmap_PartofCMEM_buf,
                                         total_size_bytes=ifmap_buf_size_bytes,
                                         word_size=word_size,
                                         active_buf_frac=rd_buf_active_frac,
                                         backing_buf_default_bw=ifmap_backing_buf_bw)

            self.ifmap_PartofCMEM_buf.set_params(backing_buf_obj=self.ifmap_port,
                                         total_size_bytes=ifmap_buf_size_bytes * 4.375,
                                         word_size=word_size,
                                         active_buf_frac=rd_buf_active_frac,
                                         backing_buf_default_bw=ifmap_backing_buf_bw)

            self.filter_PartofVMEM_buf.set_params(backing_buf_obj=self.filter_PartofCMEM_buf,
                                          total_size_bytes=filter_buf_size_bytes,
                                          word_size=word_size,
                                          active_buf_frac=rd_buf_active_frac,
                                          backing_buf_default_bw=filter_backing_buf_bw)

            self.filter_PartofCMEM_buf.set_params(backing_buf_obj=self.filter_port,
                                          total_size_bytes=filter_buf_size_bytes * 4.375,
                                          word_size=word_size,
                                          active_buf_frac=rd_buf_active_frac,
                                          backing_buf_default_bw=filter_backing_buf_bw)
        else:
    ...
    def set_read_buf_prefetch_matrices(self,
        ...
        self.ifmap_PartofVMEM_buf.set_fetch_matrix(ifmap_prefetch_mat)
        self.ifmap_PartofCMEM_buf.set_fetch_matrix(ifmap_prefetch_mat)
        self.filter_PartofVMEM_buf.set_fetch_matrix(filter_prefetch_mat)
        self.filter_PartofCMEM_buf.set_fetch_matrix(filter_prefetch_mat)

    #
    def reset_buffer_states(self):

        self.ifmap_PartofVMEM_buf.reset()
        self.ifmap_PartofCMEM_buf.reset()
        self.filter_PartofVMEM_buf.reset()
        self.filter_PartofCMEM_buf.reset()
    ...
    def service_ifmap_reads(self,
                            ...
                            incoming_cycles_arr):
        out_cycles_arr_np = self.ifmap_PartofVMEM_buf.service_reads(incoming_requests_arr_np, incoming_cycles_arr)
    ...
    def service_filter_reads(self,
                            incoming_requests_arr_np,   # 2D array with the requests
                            incoming_cycles_arr):
        out_cycles_arr_np = self.filter_PartofVMEM_buf.service_reads(incoming_requests_arr_np, incoming_cycles_arr)
    ...
    def service_memory_requests(self, ifmap_demand_mat, filter_demand_mat, ofmap_demand_mat):
        ...
        ifmap_hit_latency = self.ifmap_PartofVMEM_buf.get_hit_latency()
        filter_hit_latency = self.ifmap_PartofCMEM_buf.get_hit_latency()
        ...
        for i in tqdm(range(ofmap_lines), disable=pbar_disable):
            ...
            ifmap_cycle_out = self.ifmap_PartofVMEM_buf.service_reads(incoming_requests_arr_np=ifmap_demand_line,
            ...
            filter_cycle_out = self.filter_PartofVMEM_buf.service_reads(incoming_requests_arr_np=filter_demand_line,
                                                           incoming_cycles_arr=cycle_arr)
        ...
        if self.estimate_bandwidth_mode:
            ...
            self.ifmap_PartofVMEM_buf.complete_all_prefetches()
            self.filter_PartofVMEM_buf.complete_all_prefetches()
            self.ifmap_PartofCMEM_buf.complete_all_prefetches()
            self.filter_PartofCMEM_buf.complete_all_prefetches()
    ...
    def get_ifmap_dram_details(self):
        ...
        self.ifmap_dram_reads = self.ifmap_PartofCMEM_buf.get_num_accesses()
        self.ifmap_dram_start_cycle, self.ifmap_dram_stop_cycle \
            = self.ifmap_PartofCMEM_buf.get_external_access_start_stop_cycles()
    ...
    def get_filter_dram_details(self):
        ...
        self.filter_dram_reads = self.filter_PartofCMEM_buf.get_num_accesses()
        self.filter_dram_start_cycle, self.filter_dram_stop_cycle \
            = self.filter_PartofCMEM_buf.get_external_access_start_stop_cycles()
    ...
    def get_ifmap_dram_trace_matrix(self):
        return self.ifmap_PartofCMEM_buf.get_trace_matrix()

    #
    def get_filter_dram_trace_matrix(self):
        return self.filter_PartofCMEM_buf.get_trace_matrix()
    ...
    def get_dram_trace_matrices(self):
        dram_ifmap_trace = self.ifmap_PartofCMEM_buf.get_trace_matrix()
        dram_filter_trace = self.filter_PartofCMEM_buf.get_trace_matrix()
    ...
    def print_ifmap_dram_trace(self, filename):
        self.ifmap_PartofCMEM_buf.print_trace(filename)

    #
    def print_filter_dram_trace(self, filename):
        self.filter_PartofCMEM_buf.print_trace(filename)
